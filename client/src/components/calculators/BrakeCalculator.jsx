...
import DiscBrakeDiagram from "../2dModels/DiscBrakeDiagram";
...
const BrakeCalculator = () => {
  ...
  const calculateDiscBrake = () => {
    ...
    const F = parseFloat(actuatingForce);
    const n = parseInt(numberOfSurfaces);
    const Do = parseFloat(outerDiameter);
    const Di = parseFloat(innerDiameter);

    if ([mu, F, n, Do, Di].some(isNaN)) return alert("Please fill all numeric fields correctly!");
    if (Di >= Do) return alert("Inner Diameter must be less than Outer Diameter!");
    // console.log(Do, Di, mu, F, n);

    const ro = Do / 2;
    const ri = Di / 2;
    const rm = (2 / 3) * ((Math.pow(ro, 3) - Math.pow(ri, 3)) / (Math.pow(ro, 2) - Math.pow(ri, 2)));

    const T = n * mu * F * rm; // Braking torque

    setIsBraking(true);
    setResult({ torque: T.toFixed(2), meanRadius: rm.toFixed(2) });
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 bg-transparent rounded-2xl p-3 md:p-6 w-full max-w-7xl mx-auto">
      ...
      <div className="w-full lg:w-1/2 space-y-5">
        ...
        {/* Disc Brake Inputs */}
        {inputs.brakeType === "disc" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ["Coefficient of Friction (μ)", "coefficientOfFriction"],
              ["Actuating Force (N)", "actuatingForce"],
              ["Number of Friction Surfaces", "numberOfSurfaces"],
              ["Outer Diameter (mm)", "outerDiameter"],
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-start bg-slate-100 rounded-xl shadow-inner p-4">
        {inputs.brakeType === "shoe" ? (
          ...
          <DiscBrakeDiagram
            appliedForce={inputs.actuatingForce}
            discDiameter={inputs.outerDiameter}
            isBraking={isBraking}
            setIsBraking={setIsBraking}
            drumRotation={inputs.drumRotation}
          />
